...
export default function HeroBlob() {
  ...
  useEffect(() => {
    ...
    // Shader-based gradient blob
    const uniforms = {
      uTime: { value: 0 },
      uMouse: { value: new THREE.Vector2(0, 0) },
      uColorA: { value: new THREE.Color("#FF2E9F") },
    ...
    const vertexShader = `
      ...
      void main() {
        ...
        float mouseInfluence = 0.25 * (uMouse.x + uMouse.y);
        float displacement = n * (0.32 + mouseInfluence);
    ...
    const fragmentShader = `
      ...
      varying vec3 vPos;
      void main() {
        float t1 = clamp(vPos.x * 0.5 + 0.5, 0.0, 1.0);
        float t2 = clamp(vPos.y * 0.5 + 0.5, 0.0, 1.0);
    ...
    const glowMat = new THREE.ShaderMaterial({
      transparent: true,
      uniforms: {
        uColor: { value: new THREE.Color("#FF2E9F") },
      },
      vertexShader: `
        varying vec3 vNormal;
    ...
    const animate = () => {
      ...
      mesh.rotation.y = t * 0.2 + current.x * 0.5;
      mesh.rotation.x = current.y * 0.4;
